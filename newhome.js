...
document
    ...
    } else if (document.getElementById("video1") && document.getElementById("video1").style.display == 
"block") {
        return;
    }
    // show picture canvas
...
function webcamInference() {
    ...
    changeElementState(["picture_canvas", "prechosen_images_parent", "picture", "video1", "video", 
"video_canvas"]);
...
function processDrop(e) {
    ...
    reader.onload = function (event) {
        ...
        apiRequest(base64).then(function (predictions) {
            ...
            var predictions = predictions.map(function (prediction) {
                return {
                    bbox: { x: prediction.x, y: prediction.y, width: prediction.width, height: 
prediction.height},
                    class: prediction.class,
                    confidence: prediction.confidence,
            }});
